...
void CDebugInfo::Draw()
{
	...
	if (pSettings->GetReadOnly().szDebug == 1)
	{
		...
		pos = ImVec2(pGUI->ScaleX(25.0f), pGUI->ScaleY(465.0f - pGUI->GetFontSize()));
		pospos = ImVec2(pGUI->ScaleX(25.0f), pGUI->ScaleY(465.0f));
		pospospos = ImVec2(pGUI->ScaleX(25.0f), pGUI->ScaleY(465.0f + pGUI->GetFontSize()));

		pGUI->RenderText(pos, (ImU32)0xFFFFFFFF, true, &szStr[0]);

		int meminfo = 0;
		snprintf(&szStrMem[0], 256, "MEM: %d/%d MB", meminfo, meminfo);

		pGUI->RenderText(pospos, (ImU32)0xFFFFFFFF, true, &szStrMem[0]);
		...
		if (pGame->FindPlayerPed()->m_pPed)
		{
			...
			pGUI->RenderText(pospospos, (ImU32)0xFFFFFFFF, true, &szStrPos[0]);
		}
	}
