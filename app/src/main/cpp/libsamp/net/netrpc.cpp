...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	if(byteRejectReason == REJECT_REASON_BAD_VERSION)
		pChatWindow->AddInfoMessage("ÑÎÅÄÈÍÅÍÈÅ ÎÒÊËÎÍÅÍÎ: Íåêîððåêòíàÿ âåðñèÿ.");
	else if(byteRejectReason == REJECT_REASON_BAD_NICKNAME)
	{
		pChatWindow->AddInfoMessage("ÑÎÅÄÈÍÅÍÈÅ ÎÒÊËÎÍÅÍÎ: Íåïðèåìëèìûé NickName");
		pChatWindow->AddInfoMessage("Ïîæàëóéñòà, âûáåðèòå äðóãîé íèê îò 3 äî 20 ñèìâîëîâ");
		pChatWindow->AddInfoMessage("Ïîæàëóéñòà, èñïîëüçóéòå òîëüêî a-z, A-Z, 0-9");
		pChatWindow->AddInfoMessage("Èñïîëüçóéòå /quit äëÿ âûõîäà");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_MOD)
	{
		pChatWindow->AddInfoMessage("ÑÎÅÄÈÍÅÍÈÅ ÎÒÊËÎÍÅÍÎ: Íåêîððåêòíàÿ âåðñèÿ ìîäà.");
	}
	else if(byteRejectReason == REJECT_REASON_BAD_PLAYERID)
	{
		pChatWindow->AddInfoMessage("ÑÎÅÄÈÍÅÍÈÅ ÎÒÊËÎÍÅÍÎ: Íå óäàëîñü âûäåëèòü ñëîò äëÿ èãðîêà.");
	}
