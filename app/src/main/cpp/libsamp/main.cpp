...
Java_com_sasamp_startMenu_GameMenuStart_connectToServer(JNIEnv *env, jobject thiz, jstring nick,
    ...
    // Verificar se as strings não são nulas antes de usá-las
    if (utfip != nullptr && utfnick != nullptr) {
        ...
        CSettings::m_Settings.szNickName[sizeof(CSettings::m_Settings.szNickName) - 1] = '\0'; // Garantir terminação nula

        strncpy(CSettings::m_Settings.cHost, utfip, sizeof(CSettings::m_Settings.cHost) - 1);
        CSettings::m_Settings.cHost[sizeof(CSettings::m_Settings.cHost) - 1] = '\0'; // Garantir terminação nula
