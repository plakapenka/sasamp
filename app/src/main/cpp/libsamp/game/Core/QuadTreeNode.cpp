...
CQuadTreeNodePool* CQuadTreeNode::ms_pQuadTreeNodePool = nullptr;

void CQuadTreeNode::InjectHooks() {
    CHook::Write(g_libGTASA + 0x00679568, &CQuadTreeNode::ms_pQuadTreeNodePool);
}
...
CQuadTreeNode::CQuadTreeNode(const CRect& size, int32_t startLevel) : m_ItemList(), m_apChildren{ nullptr }
{
    m_Rect = size;
    m_nLevel = startLevel;
}
...
int32_t CQuadTreeNode::FindSector(const CRect& rect)
{
...
// Returns -1 if not found
int32_t CQuadTreeNode::FindSector(const CVector2D& posn)
{
    const auto center = m_Rect.GetCenter();
    if (!m_nLevel)
        return -1;
...
// 0x5526A0
bool CQuadTreeNode::InSector(const CRect& rect, int32_t sector) const
{
    if (!m_nLevel)
        return false;
...
CRect CQuadTreeNode::GetSectorRect(int32_t sector) const
{
