...
inline uint32_t GetCommonSize(uint32_t size) {
    return (size >> 4) - 1;
}
...
// 0x72F180
void CMemoryHeap::Init(void* mem, uint32_t size, bool bInitSizes) {
    m_nMemId   = -1;
    m_nMemUsed = 0;
    ...
    m_FreeListEnd.m_nSize  = std::numeric_limits<int32_t>::max();
    ...
    if (bInitSizes) {
        ...
        int32_t asize = MEMORY_HEAP_ALIGN;
...
HeapFreeBlockDesc* CMemoryHeap::_JoinFreeBlocks(HeapFreeBlockDesc* desc) {
    ...
    if (next && !next->m_bInUse) {
        ...
        next = (HeapFreeBlockDesc*)((uint8_t*)next + next->m_nSize + sizeof(HeapBlockDesc));
    }
    assert(next);
    ...
    // recount total size
    desc->m_nSize = (uint8_t*)next - (uint8_t*)desc - sizeof(HeapBlockDesc);
...
// 0x72EEF0
void* CMemoryHeap::Realloc(void* memory, uint32_t size) {
    if (!memory) {
        return Malloc(size);
    }
    ...
    // extend block
    if (int32_t(nBlockSize) > GetBlockDesc(memory)->m_nSize) {
        // __DUMMY();
...
// 0x72ED80
void* CMemoryHeap::Malloc(uint32_t size) {
    auto blockSize = size;
    if (size & (MEMORY_HEAP_ALIGN - 1)) {
    ...
    while (blockSize > uint32_t(block->m_nSize)) {
        block = block->m_Next;
    ...
    assert((uint32_t)block->m_nSize < GetHeapSize() && "Heap is corrupted!");
...
// 0x72EC80
void CMemoryHeap::FillInBlockData(HeapBlockDesc* desc, HeapBlockDesc* nextDesc, uint32_t size) {
    // __DUMMY();
    ...
    if (next < &nextDesc[-1]) {
        ...
        next->m_nSize = (uint8_t*)nextDesc - (uint8_t*)next - sizeof(HeapBlockDesc);
        next->m_PrevBlock = desc;
        next->m_bInUse = false;
    ...
    if (next < nextDesc) {
        desc->m_nSize = (uint8_t*)nextDesc - (uint8_t*)desc - sizeof(HeapBlockDesc);
    }
...
// 0x72E980
uint32_t CMemoryHeap::GetSizeOfHoles() {
    uint32_t nHolesSize = 0;
...
void CMemoryHeap::IntegrityCheck() {
    CHook::CallFunction<void>(g_libGTASA + 0x005D2716 + 1, this);
}
...
void* CMemoryHeap::MoveMemory(void* memory, void** _ppUnk, int32_t _iUnk) {
    assert(true && "NOT IMPLEMENTED");
    return memory;
}
...
void CMemoryHeap::ParseHeap() {
    ...
    while (block < m_LastBlock) {
        ...
        for (uint32_t size = block->m_nSize >> 4; size; --size) {
            if ((k & 63) == 0) {
                sprintf(buffer, "\n%5dK:", k / 64);
                CFileMgr::Write(file, buffer, 8);
            }
    ...
    while (p < m_LastBlock) {
        if (p->m_nMemId != -1) {
            if (memory >= p->_GetBlockData()) {
                if ((int8_t*)memory < (int8_t*)p->_GetBlockData() + p->m_nSize) {
                    return p;
                }
            }
        }
